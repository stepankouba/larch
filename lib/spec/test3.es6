...
const methods = { round: 'function round(value) {value = value[0];return [Math.round(value.total / value.days.length * 100) / 100];}' }; 

console.log(JSON.stringify(transform(source, template, JSONParser.evaluate(methods)), null, 4));
